...
public class FileFunctionExecutor implements FunctionExecutor {
    ...
    @Comment("下载Url资源")
    @Example("${file.download('e:/downloadPath',urls, true)}")
    public static void download(String path, List<String> urls, boolean skipTLS) throws IOException {
        if (!CollectionUtils.isEmpty(urls)) {
            for (String url : urls) {
                FileUtils.downloadFile(path, url, true, skipTLS);
            }
        }
    }
    ...
    @Comment("下载Url资源")
    @Example("${file.download('e:/downloadPath',urls, true)}")
    public static void download(String path, String url, boolean skipTLS) throws IOException {
        if (url != null) {
            FileUtils.downloadFile(path, url, true, skipTLS);
        }
    }
